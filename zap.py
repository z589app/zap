...
def cmd_alias(config, args):
    ...
    if sub == "add":
        ...
        if len(rest) == 1 and re.match(r"^-\d+$", rest[0]):
            ...
            cmd_str = escape_special(cmd_str)
...
def escape_special(cmd_str):
    """{ , } , % をリテラルとして保存するため \\{ , \\} , \\% にエスケープする。
    履歴(実行済みの生コマンド、中括弧・%はすべてリテラル)をaliasとして登録する際、
    プレースホルダー/common参照として誤解釈されないようにするために使う"""
    return cmd_str.replace("{", "\\{").replace("}", "\\}").replace("%", "\\%")
...
def run_segment(config, tokens, dry, silent):
    ...
    if tokens[0].startswith("@"):
        ...
        cmd_str = " ".join(tokens)
        cmd_str = expand_common(config, cmd_str)
        run_and_record(config, resolve_default_placeholder(cmd_str), dry, silent)
